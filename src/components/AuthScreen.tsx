...
export default function AuthScreen({ onAuthenticated, initialMode = 'login', onBackToLanding }: AuthScreenProps) {
  ...
  const handleForgotPassword = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) {
        throw new Error(result.error || 'Gagal mengirim permintaan reset password.');
      }

      setSuccessMessage(
        'Jika email tersebut terdaftar, kami telah mengirim tautan reset password. Silakan cek inbox atau folder spam.'
      );
      setEmail('');
    } catch (e: any) {
          ...
          <div className="relative flex items-center gap-3 sm:gap-4">
            ...
            <div>
              ...
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-black tracking-tight mt-1 leading-tight">
                ...
                <span className="text-teal"> {mode === 'register' ? 'Gratis' : ''}</span>
              </h1>
            </div>
          </div>
          ...
          {mode === 'forgot' ? (
            ...
            <form onSubmit={handleForgotPassword} className="space-y-4 sm:space-y-5">
              <p className="text-sm text-slate-600 leading-relaxed">
                Masukkan alamat email yang terdaftar. Kami akan mengirim tautan untuk mereset password Anda.
              </p>
              ...
              {/* Success Message */}
              {successMessage && (
                <div className="p-3 sm:p-4 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs sm:text-sm font-medium flex items-start gap-2">
              ...
              {mode === 'login' && (
                <div className="text-center">
                  <button
                    ...
                    className="text-xs sm:text-sm text-teal hover:text-teal-dark font-semibold transition-colors cursor-pointer"
                  >
                    Lupa Password?
                  </button>
                </div>
